...
function compile {
    echo "$1" | ./build/8cc -o tmp.s - || fail "Failed to compile $1"
    gcc -o tmp.out tmp.s
...
function testastf {
    result="$(echo "$2" | ./build/8cc -o - -fdump-ast -w -)"
...
function testcpp {
    echo "$2" | ./build/8cc -o - -E $3 - > tmp.s || fail "Failed to compile $1"
    assertequal "$(cat tmp.s)" "$1"
}


function testfail {
    echo "$expr" | ./build/8cc -o /dev/null - 2> /dev/null
    expr="int f(){$1}"
    echo "$expr" | ./build/8cc -o /dev/null $OPTION - 2> /dev/null
